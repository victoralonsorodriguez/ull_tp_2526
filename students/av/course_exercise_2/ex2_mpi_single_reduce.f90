...
program ex2_mpi
    ...
    allocate(buffer_send(3 * n_particles))
    allocate(buffer_recv(3 * n_particles))
    ...
    do while (t < t_end)
        ...
        do i = 1, n_particles
            buffer_send((i-1)*3 + 1) = acc_local(i)%x
            buffer_send((i-1)*3 + 2) = acc_local(i)%y
            buffer_send((i-1)*3 + 3) = acc_local(i)%z
        end do
        ...
        !--- unpacking results ---!
        do i = 1, n_particles
            acc_global(i)%x = buffer_recv((i-1)*3 + 1)
            acc_global(i)%y = buffer_recv((i-1)*3 + 2)
            acc_global(i)%z = buffer_recv((i-1)*3 + 3)
        end do
